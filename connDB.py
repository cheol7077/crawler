...
def insert (board_id, title, content, date, url, hits, commentCnt, communityID) :
    ...
    with conn.cursor() as cursor:
        ...
        sql = '''INSERT INTO board (board_id, title, content, date, url, hits, commentCnt, communityID)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)'''
        cursor.execute(sql, (board_id, title, content, date, url, hits, commentCnt, communityID))
        conn.commit()
...
def insertThumbnail(file_name, board_id):
    conn = pymysql.connect(host='localhost',
        user = 'root',
        password = 'hubhub',
        db='community',
